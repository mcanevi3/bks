...
for i,pval in enumerate(pvec):
    ...
    mpvec[i]=pval/(zeta*wn)

index=np.argmax(mpvec)
mval=mpvec[index]
pval=pvec[index]
...
rlocus_plot(Gzs,kvec=np.logspace(-2,2,1500))
plt.xlim([-5*zeta*wn,1])
plt.plot([-zeta*wn,-zeta*wn],[-1,1],'k')
...
kpival=kival/(10*zeta*wn)
kpdval=kpval-kpival
Fspd=control.tf([kdval,kpdval],1)
Fspi=control.tf([kpival,kival],[1,0])
Gs=control.tf(num_gs,den_gs)
Ts1=control.feedback(Gs,Fspd)
Ts=control.feedback(Fspi*Ts1,1)
print(Ts)
